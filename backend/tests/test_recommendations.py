...
def test_recommendations_rank_backprop_and_are_idempotent(tmp_path):
    ...
    item = graph(); store.save_graph(item)
    session = LearningSession(id="session-neural", learner_id="local", graph_id=item.id, current_concept_id="intro", created_at=utc_now(), updated_at=utc_now())
