...
from .assessment_generation import evaluate, generate_item
...
class QuizService:
    ...
    def create(self, owner, request: QuizCreate, connection, quiz_id):
        ...
        quiz = {"id": quiz_id, "sessionId": session.id, "conceptIds": concepts, "graphId": graph.id,
                ...
                "attempts": [], "presentations": [], "revision": 1}
    ...
    def public(self, owner, quiz_id):
        ...
        return {**quiz, "current": current, "attempts": history}
    ...
    def prepare(self, owner, quiz_id, revision):
        ...
        manifest = save_manifest(self.store, owner, session.id, concept.title, sources)
        ...
        context = {"conceptIds": [concept_id], "concept": concept.title, "objective": session.goal,
                   "difficulty": difficulty, "sources": sources, "evidence": canonical_evidence(self.store, owner, graph).model_dump(mode="json"),
                   "recentFeedback": recent["feedback"] if recent else None, "questionNumber": len(first) + 1}
        item, check = generate_item(self.provider, context, previous)
        ...
        return quiz, item, {**public, "id": item_id, "check": check, "contextId": manifest["id"], "provider": self.provider.provider_name}, presentation
